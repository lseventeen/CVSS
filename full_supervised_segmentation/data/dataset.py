...
class CVSS_test_dataset(CVSS_train_dataset):
    def __init__(self, config, images_path, labels_path):
        ...
        self.img_list, self.gt_list  = self.read_image(self.images_path,self.labels_path)
        # self.img_list, self.gt_list = self.preprocess()
        # self.img_list = self.read_image(self.images_path)
        # self.gt_list = self.read_label(self.labels_path)
    ...
    def get_patch(self,image_list, patch_size, stride):
        ...
        for image in image_list:
            image = F.pad(torch.from_numpy(image).float(),(0, pad_w, 0, pad_h), "constant", 0)
