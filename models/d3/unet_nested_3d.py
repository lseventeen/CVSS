...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, n_concat=2, kernel_size=2):
        ...
        if is_deconv:
            self.up = nn.ConvTranspose3d(in_size, out_size, kernel_size=kernel_size, stride=kernel_size, padding=0)
        else:
...
class UNet_Nested_3D(nn.Module):
    ...
    def __init__(self, num_channels=1, num_classes=2, feature_scale=2, is_deconv=True, is_batchnorm=True, is_ds=True):
        ...
        # upsampling
        self.up_concat01 = unetUp(filters[1], filters[0], self.is_deconv, kernel_size=(1,2,2))
        ...
        self.up_concat02 = unetUp(filters[1], filters[0], self.is_deconv, 3, kernel_size=(1,2,2))
        self.up_concat12 = unetUp(filters[2], filters[1], self.is_deconv, 3)
        self.up_concat22 = unetUp(filters[3], filters[2], self.is_deconv, 3)

        self.up_concat03 = unetUp(filters[1], filters[0], self.is_deconv, 4, kernel_size=(1,2,2))
        self.up_concat13 = unetUp(filters[2], filters[1], self.is_deconv, 4)
        
        self.up_concat04 = unetUp(filters[1], filters[0], self.is_deconv, 5, kernel_size=(1,2,2))
