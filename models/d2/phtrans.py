...
class PHTrans(nn.Module):
    def __init__(self, num_classes = 1, 
                 ...
                 pool_op_kernel_sizes=[[1,2,2],[1,2,2],[1,2,2],[2,2,2]],
        ...
        for ds in range(self.num_pool):
            self.seg_outputs.append(DeepSupervision(min((base_num_features * feat_map_mul_on_downscale **ds), max_num_features), num_classes))
        ...
        for i_layer in range(self.num_pool+1):  # 0,1,2,3,4,5
            layer = BasicLayer(num_stage=i_layer, 
                               ...
                               image_channels=num_channels, num_conv_per_stage=num_conv_per_stage,
...
class DeepSupervision(nn.Module):
    def __init__(self, dim, num_classes):
        ...
        self.conv1 = nn.Conv3d(dim, 1, kernel_size=1,bias=False)
        self.conv2 = nn.Conv2d(8, num_classes, kernel_size=1,bias=False)
    def forward(self, x):
        x = self.conv1(x)
