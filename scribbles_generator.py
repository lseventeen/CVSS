...
def scrible_2d(lab, iteration=[4, 10]):    
    struct = ndimage.generate_binary_structure(2, 2)
...
def scribble4class(label, class_id, class_num, iteration=[4, 10], cut_branch=True):
    ...
    if cut_branch and class_id != 0:
        cut = Cutting_branch()
        if sk_map.sum() >= 1:
            sk_map = cut(sk_map, seg_lab=label)
    if class_id == 0:
        class_id = class_num
...
def data_preprocess(labels_path, scribbles_label_path, is_overwrite=True) :
       
    if is_overwrite and isdir(scribbles_label_path):
        shutil.rmtree(scribbles_label_path)
    os.makedirs(scribbles_label_path, exist_ok=True)
