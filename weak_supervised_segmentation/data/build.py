...
def build_train_loader(config):
    ...
    elif config.SCRIBBLE_TYPE == "centerline":
        labels_path = config.DATASET.CENTERLINE_LABEL_PATH
    else:
        raise NotImplementedError(f"Unkown param: {config.SCRIBBLE_TYPE}")

    train_dataset = CVSS_train_dataset(config,images_path=config.DATASET.TRAIN_IMAGE_PATH, labels_path=labels_path)
...
def build_test_loader(config):
    test_dataset = CVSS_test_dataset(config,images_path=config.DATASET.TEST_IMAGE_PATH, labels_path=config.DATASET.TEST_LABEL_PATH)
