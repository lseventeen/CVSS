...
class PLM(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.UN_maxpooling = nn.MaxPool3d(kernel_size = (2,1,1),stride=(2,1,1))
...
class IPN(nn.Module):
    ...
    def __init__(self, num_channels=1, num_classes=1, feature_scale=4):
        ...
        self.RC_conv = nn.Sequential(
            ...
            nn.Conv3d(filters[4], filters[2], kernel_size=3,padding=1, bias=False),
           
            nn.LeakyReLU(0.1, inplace=True),

            nn.Conv3d(filters[2], filters[0], kernel_size=3, padding=1, bias=False),
           
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv3d(filters[0], num_classes, kernel_size=3, padding=1, bias=False),
